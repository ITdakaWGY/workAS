...
import com.as.occupationaldseases.utils.*;
...
public class GrjbxxService {
    ...
    @Resource
    GrjbxxZybMapper grjbxxZybMapper;
    @Resource
    ...
    public GrjbxxResult insertAndUpdate(Grjbxx grjbxx){
        ...
        if (grjbxx1!=null) {
            ...
            if (grjbxx1.getPersonalid()==null||grjbxx1.getPersonalid().length()==0)
                grjbxx.setPersonalid(Long.toString(System.currentTimeMillis()));

            //查询最大值加1生成条码号
            if(!(grjbxx.getTmh()!=null&&grjbxx.getTmh().length()>0)){
                String maxTmh = grjbxxMapper.getMaxTmh();
                String newTmh= String.valueOf( Long.valueOf(maxTmh)+1);
                grjbxx.setTmh(newTmh);
            }

            grjbxxMapper.updateById(grjbxx);
            //更新项目表的条码号
            if(grjbxx.getTmh()!=null&&grjbxx.getTmh()!=""){
                QueryWrapper<Grjbxx_xm> wrapper2 = new QueryWrapper<>();
                Grjbxx_xm grjbxxXm2 = new Grjbxx_xm();
                grjbxxXm2.setTmh(grjbxx.getTmh());
                grjbxxXmMapper.update(grjbxxXm2,wrapper2.eq("sfz",grjbxx.getSfz()));
    ...
    public QueryResponseResult selectWhole(Grjbxx grjbxx){
        ...
        if (grjbxxList.get(0).getPersonalid()!=null&&grjbxxList.get(0).getPersonalid().length()>0){
            ...
            List<Grjbxx_zyb> grjbxx_zybList = grjbxxZybMapper.selectByMap(map);
            ...
            grjbxxWhole.setGrjbxxZyb(grjbxx_zybList);
        }
        map.remove("personalid");
    ...
    private void delSchedule(Map<String,Object> map){
        ...
        grjbxxZybMapper.deleteByMap(map);
    }
    /**
     * 新增所有附表
     * @param grjbxxWhole
    ...
    private   void   addSchedule(GrjbxxWhole grjbxxWhole){
        ...
        //职业病
        if (isNullObject(grjbxxWhole.getJbxxZyb())) {
            Grjbxx_zyb jbxxzyb = grjbxxWhole.getJbxxZyb();
            String[] illnexxname = jbxxzyb.getIllnessname().split(",");
            String[] diagnosiscompany = jbxxzyb.getDiagnosiscompany().split(",");
            String[] diagnosisdata = jbxxzyb.getDiagnosisdata().split(",");
            String[] sfrecovery = jbxxzyb.getSfrecovery().split(",");
            for (int i = 0; i < illnexxname.length; i++) {
                Grjbxx_zyb grjbxxZyb = new Grjbxx_zyb();
                grjbxxZyb.setDiagnosiscompany(diagnosiscompany[i]);
                grjbxxZyb.setDiagnosisdata(diagnosisdata[i]);
                grjbxxZyb.setIllnessname(illnexxname[i]);
                grjbxxZyb.setSfrecovery(sfrecovery[i]);
                grjbxxZyb.setPersonalid(grjbxxWhole.getGrjbxx().getPersonalid());
                grjbxxZyb.setId(null);
                grjbxxZybMapper.insert(grjbxxZyb);
            }
...
//        current = current - 1;
    ...
    public List<ProjectState> getProject(String tmh){
        String str = "症状问询内科常规检查神经系统常规检查外科常规检查";
        List<Grjbxx_xm> grjbxx_xms = grjbxxXmMapper.findByTmh(tmh);//得到个人项目
        List<Grjbxx> grjbxxes = grjbxxMapper.findByTmh(tmh);//得到状态
        if (StringUtil.isEmpty(grjbxxes.get(0).getTjflagzz())){
            grjbxxes.get(0).setTjflagzz("0");
        }
        if (StringUtil.isEmpty(grjbxxes.get(0).getTjflagnk())){
            grjbxxes.get(0).setTjflagnk("0");
        }
        if (StringUtil.isEmpty(grjbxxes.get(0).getTjflagsjk())){
            grjbxxes.get(0).setTjflagsjk("0");
        }
        if (StringUtil.isEmpty(grjbxxes.get(0).getTjflagwk())){
            grjbxxes.get(0).setTjflagwk("0");
        }
        ...
        for (int i = 0; i < jcxmcode.length; i++) {
            ...
            String projctName = diseaseItem.getIteamName();
            projectState.setProjctName(projctName);
            if (str.contains(projctName)){
                if ("症状问询".equals(projctName) && "1".equals(grjbxxes.get(0).getTjflagzz())){
                    projectState.setState("1");
                }else if ("内科常规检查".equals(projctName) && "1".equals(grjbxxes.get(0).getTjflagnk())){
                    projectState.setState("1");
                }else if ("神经系统常规检查".equals(projctName) && "1".equals(grjbxxes.get(0).getTjflagsjk())){
                    projectState.setState("1");
                }else if ("外科常规检查".equals(projctName) && "1".equals(grjbxxes.get(0).getTjflagwk())){
                    projectState.setState("1");
                }else {
    ...
    public R downloadFile(HttpServletResponse response) throws UnsupportedEncodingException {
        System.out.println("文件名称："+FullPath);
