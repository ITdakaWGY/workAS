...
layui.define(["element","jquery"],function(exports){
	...
	if($(".navBar").html() == ''){
		var _this = this;
		$(".navBar").html(navBar(navs)).height($(window).height()-100);
			...
			if(_this.find("i.iconfont").attr("data-icon") != undefined){
				...
				if(that.hasTab(_this.find("cite").text()) == -1 && _this.siblings("dl.layui-nav-child").length == 0){
					...
					element.tabAdd(tabFilter, {
				        title : title,
//				        content :"<iframe src='"+_this.attr("data-url")+"' data-id='"+tabIdIndex+"' name='rightDom'></frame>",
				        content:'<iframe    name="rightDom'+tabIdIndex+'" data-id="'+tabIdIndex+'" src="'+_this.attr("data-url")+'" ></frame>',
				        id : new Date().getTime()
				    })

					//当前窗口内容
					var curmenu = {
						"icon" : _this.find("i.iconfont").attr("data-icon")!=undefined ? _this.find("i.iconfont").attr("data-icon") : _this.find("i.iconfont").attr("data-icon"),
