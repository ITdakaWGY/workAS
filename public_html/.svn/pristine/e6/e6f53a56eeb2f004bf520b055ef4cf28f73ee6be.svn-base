	...
	layui.use(['layer', 'form', 'laydate'], function() {
		...
		$('body').on('click', '#AddBtn', function() {
			...
			top.layer.open({
				...
				shade: 0.8,
				area: ['50%', '60%'],
				content: 'page/sysorg/sysOrgAdd.html?frameId='+frameId,
			});
		})
		...
		$('body').on('click', '.editBtn', function() {
			...
			top.layer.open({
				...
				shade: 0.8,
				area: ['70%', '50%'],
				content: 'page/sysorg/sysOrgedit.html?id=' + id+'&frameId='+frameId,
			});
		})
...
function getData(param) {
	...
	var urls = 'sysOrg/list/1/' + pageSize;
...
function getPageData(page, data) {
	...
	var urls = 'sysOrg/list/' + page + '/' + pageSize;
	var loading = layer.msg('正在查询...', {
		icon: 16,
		shade: 0.2,
		time: false
