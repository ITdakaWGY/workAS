$(function() {
	...
	layui.use(['layer', 'form', 'laydate'], function() {
		...
		$('body').on('click', '.deleteBtn', function() {
			var hazardCode = $(this).attr('data-hazardCode');
...
function getData(jobcode) {
	var urls = 'Hazardinfo/hazardJobList/'+jobcode+'/1/' + pageSize;
...
function setdata(result) {
	...
	for(var i = 0; i < result.length; i++) {
		html += `<tr>
								...
								<th>
									<span class="layui-btn chaxunBtn deleteBtn" data-hazardCode="${result[i].hazardCode}">删除</span>
								</th>
							</tr>`;
	}
	$('#tbody').html(html);
...
function getPageData(page, jobcode) {
	...
	var urls = 'Hazardinfo/hazardJobList/'+jobcode+'/'+ page + '/' + pageSize;
	var loading = layer.msg('正在查询...', {
		icon: 16,
		shade: 0.2,
		time: false
